...
namespace Unlakki.Bns.Launcher.Core.Services
{
    ...
    class GameAuthProvider : IGameAuthProvider
    {
        ...
        public async Task<GameTokenCode> GetGameTokenCode(string accessToken)
        {
            ...
            GameAccount gameAccount = JsonConvert.DeserializeObject<WebSocketResponse<GameAccount[]>>(gameAccountsString).Result[0];
            ...
            GameTokenCode gameTokenCode = JsonConvert.DeserializeObject<WebSocketResponse<GameTokenCode>>(gameTokenCodeString).Result;
