  ...
  public static class RegistryHelper
  {
    ...
    public static bool TryRegisterUninstallInfo(string appKey, RegistryUninstallInfo info)
    {
      try
      {
        ...
        using (RegistryKey registryKey1 = _defaultRootKey.OpenSubKey(uninstallRegistryFolder, true)
          ?? _defaultRootKey.CreateSubKey(uninstallRegistryFolder))
        {
          ...
          registryKey1.SetValue(
            RegistryConstants.Uninstall.InstallLocation, info.InstallationPath ?? string.Empty);
          registryKey1.SetValue(RegistryConstants.Uninstall.DisplayIcon,
            info.IconPath ?? string.Empty);
          registryKey1.SetValue(
            RegistryConstants.Uninstall.UninstallString, info.UninstallCommand ?? string.Empty);
          registryKey1.SetValue(
    ...
    public static RegistryUninstallInfo GetUninstallInfo(string key)
    {
      ...
      using (RegistryKey registryKey = _defaultRootKey.OpenSubKey(uninstallRegistryFolder, true)
        ?? _defaultRootKey.CreateSubKey(uninstallRegistryFolder))
      {
        if (registryKey == null)
      ...
      string launcherKey)
    {
        ...
        ?? _defaultRootKey.CreateSubKey(launcherRegistryFolder))
      {
        ...
        string str1 = registryKey.GetValue(
        ...
        string str2 = registryKey.GetValue(
        ...
        return new RegisterLauncherSoftwareInfo()
        {
          ...
          LauncherRegion = str3,
          InstallationDate = ParseRegistryDate(str1, RegistryConstants.Format.DateFormat)
        };
      }
    }
    ...
    public static bool TryRegisterLauncherSoftwareData(RegisterLauncherSoftwareInfo softwareInfo)
    {
      try
      {
        ...
        using (RegistryKey registryKey1 = _defaultRootKey.OpenSubKey(launcherRegistryFolder, true)
          ?? _defaultRootKey.CreateSubKey(launcherRegistryFolder))
        {
          if (registryKey1 == null)
            return false;
          ...
          if (softwareInfo.OldGamesTaken.HasValue)
          {
            ...
            bool flag = true;
            ValueType local = oldGamesTaken2.GetValueOrDefault() == flag & oldGamesTaken2.HasValue
              ? 1
              : 0;
          ...
          if (!string.IsNullOrWhiteSpace(softwareInfo.InstallationPath))
            registryKey1.SetValue(
              RegistryConstants.Software.Path, softwareInfo.InstallationPath ?? string.Empty);
      ...
      string launcherId)
    {
      try
      {
          ...
          ?? _defaultRootKey.CreateSubKey(launcherRegistryFolder))
        {
          if (registryKey == null)
            return false;
          if (string.IsNullOrWhiteSpace(registryKey.GetValue(
      ...
      string gameName)
    {
      try
      {
        string gameRegistryFolder = GetSoftwareGameRegistryFolder(publisher, launcherKey, gameName);
        using (RegistryKey registryKey = _defaultRootKey.OpenSubKey(gameRegistryFolder, true))
        {
          ...
          return new RegisterGameSoftwareInfo()
          {
            ...
            InstallationPath = registryKey.GetValue(RegistryConstants.Software.Path)?.ToString(),
            Version = registryKey.GetValue(RegistryConstants.Software.Version)?.ToString(),
            InstallationDate = new DateTime?(ParseRegistryDate(str, RegistryConstants.Format.DateFormat) ?? DateTime.Now)
          };
        }
      }
      catch (BadRegistryPathPart)
    ...
    {
      return Path.Combine(
        GetSoftwareGamesRegistryFolder(publisher, launcherKey), GetValidPathPart(gameName) ?? "");
    }
