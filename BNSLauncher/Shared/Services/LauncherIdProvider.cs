...
namespace Unlakki.Bns.Launcher.Shared.Services
{
    ...
    public class LauncherIdProvider : ILauncherIdProvider
    {
        ...
        {
            _launcherId = new Lazy<string>(new Func<string>(GetCore), LazyThreadSafetyMode.ExecutionAndPublication);
        }
        ...
        private string GetCore()
        {
            try
            {
                string launcherId = _launcherInSystemRegistrator.GetLauncherSoftwareInfo("4game2.0").LauncherId;
