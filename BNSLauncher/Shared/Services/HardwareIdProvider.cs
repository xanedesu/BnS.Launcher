...
namespace BNSLauncher.Shared.Providers
{
    ...
    public sealed class HardwareIdProvider : IHardwareIdProvider
    {
        ...
        private string GetHardwareId()
        {
            ...
            string str1 = motherBoardId != null ? motherBoardId.Replace(" ", "").SubstringOrSelf(0, 53) : null;
        ...
        private string GetMotherBoardId()
        {
            try
            {
                return new ManagementObjectSearcher("SELECT * FROM Win32_BaseBoard").Get().OfType<ManagementObject>().Select(v => v["SerialNumber"]?.ToString()).FirstOrDefault(v => v != null);
            }
            catch (Exception)
            {
                return string.Empty;
        ...
        private string GetCpuId()
        {
            try
            {
                return new ManagementObjectSearcher("Select * From Win32_processor").Get().OfType<ManagementObject>().Select(v => v["ProcessorId"]?.ToString()).FirstOrDefault(v => v != null);
            }
            catch (Exception)
            {
                return string.Empty;
